...
function comment_update(form, button){
	...
	$(form).ajaxSubmit({
		...
		success: ajaxFormSuccess(function(json, statusText, xhr, jqForm){
			...
			$('> .message', comment).html(json.message);
			$('> .info time', comment).html(json.time_changed + ' <span class="time_changed">(комментарий был изменён)</span>');
			$('.comment_item .reply a').show();
			$("pre code").each(function() {hljs.highlightBlock(this)});
		}, null, true)
